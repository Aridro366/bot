...
async def on_member_join(member: discord.Member):
    ...
    if channel:
        # Main text
        welcome_message = (
            f"🎉 Welcome to Royals Empire 🍻\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"Hey {member.mention} 👑\n"
            f"You’ve just stepped into the kingdom of vibes, loyalty, and legends.\n"
            f"We're hyped to have you with us!\n\n"
            f"✨ Make sure to:\n"
            f"🔹 Verify yourself in <#1401306747922284625>  (Required to unlock the server)\n"
            f"🔹 Read the rules in <#1396670355858391092>\n"
            f"🔹 Get your roles from <#1396726703564525688>\n"
            f"🔹 Say hi in <#1396801885322739743>\n\n"
            f"📣 Reminder: Be respectful, stay chill, and vibe with your fellow Royals.\n"
            f"Need help? Ping <@&1396679140303835278> <@&1396810929110388806>\n"
            f"Enjoy the music, games & chill vibes!\n"
            f"Welcome again, Your Majesty! 🎈\n"
            f"━━━━━━━━━━━━━━━━━━━"
        )
        ...
        embed.set_image(
            url="https://cdn.discordapp.com/attachments/1338401226357870654/1410590875683127368/standard_3.gif"
        )
...
class HelpView(discord.ui.View):
    ...
    async def moderation(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="/ban | ?ban", value="Ban a member", inline=False)
        embed.add_field(name="/unban | ?unban", value="Unban a member", inline=False)
        embed.add_field(name="/kick | ?kick", value="Kick a member", inline=False)
        embed.add_field(name="/timeban | ?timeban", value="Temporarily ban a member", inline=False)
        embed.add_field(name="/purge | ?purge", value="Delete multiple messages", inline=False)
    ...
    async def utility(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="/serverinfo | ?serverinfo", value="Server information", inline=False)
        embed.add_field(name="/memberinfo | ?memberinfo", value="Member information", inline=False)
        embed.add_field(name="/roleinfo | ?roleinfo", value="Role information", inline=False)
        embed.add_field(name="/avatar | ?avatar", value="Get user's avatar", inline=False)
        embed.add_field(name="/announce | ?announce", value="Send an announcement", inline=False)
        await interaction.response.edit_message(embed=embed, view=self)
    ...
    async def fun(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="/roast | ?roast", value="Roast a member", inline=False)
        embed.add_field(name="/joke | ?joke", value="Tell a joke", inline=False)
        embed.add_field(name="/hug 🤗 | ?hug", value="Hug someone", inline=False)
        embed.add_field(name="/kiss 😘 | ?kiss", value="Kiss someone", inline=False)
        embed.add_field(name="/slap 👋 | ?slap", value="Slap someone", inline=False)
        embed.add_field(name="/punch 🥊 | ?punch", value="Punch someone", inline=False)
        embed.add_field(name="/cuddle 🛏 | ?cuddle", value="Cuddle someone", inline=False)
        embed.add_field(name="/love ❤ | ?love", value="Show love", inline=False)
        embed.add_field(name="/lick 👅 | ?lick", value="Lick someone", inline=False)
        embed.add_field(name="/pamper 🌸 | ?pamper", value="Pamper someone", inline=False)
        embed.add_field(name="/kill 💀 | ?kill", value="Playfully kill someone", inline=False)
        embed.add_field(name="/coinflip 🪙 | ?coinflip", value="Flip a coin", inline=False)
        embed.add_field(name="/ship ❤️ | ?ship", value="Ship two users", inline=False)
        embed.add_field(name="/rate ⭐ | ?rate", value="Rate someone or something", inline=False)
        await interaction.response.edit_message(embed=embed, view=self)
...
# ----------------- Coinflip -----------------
async def do_coinflip(ctx_or_inter):
    result = random.choice(["Heads", "Tails"])
    embed = Embed(title="🪙 Coin Flip",
...
async def coinflip_slash(interaction: discord.Interaction):
    await do_coinflip(interaction)

@bot.command(name="coinflip")
async def coinflip_prefix(ctx):
    await do_coinflip(ctx)

# ----------------- Rate -----------------
async def do_rate(ctx_or_inter, thing: str):
    rating = random.randint(0, 10)
    embed = Embed(title="⭐ Rating Machine",
...
@bot.tree.command(name="rate", description="Rate something!", guild=discord.Object(id=GUILD_ID))
async def rate_slash(interaction: discord.Interaction, thing: str):
    await do_rate(interaction, thing)

@bot.command(name="rate")
async def rate_prefix(ctx, *, thing: str):
    await do_rate(ctx, thing)

# ----------------- Ship -----------------
async def do_ship(ctx_or_inter, user1: discord.Member, user2: discord.Member):
    percent = random.randint(0, 100)
    if percent > 75:
        msg = "💖 A perfect match!"
        color = discord.Color.red()
    elif percent > 50:
        msg = "💞 Could work out nicely!"
        color = discord.Color.pink()
    elif percent > 25:
        msg = "💔 Not looking too good..."
        color = discord.Color.orange()
    else:
        msg = "🚪 Better stay friends."
        color = discord.Color.greyple()

    embed = Embed(title="💘 Love Calculator",
                  description=f"Shipping {user1.mention} and {user2.mention}...\n"
...
@bot.tree.command(name="ship", description="Ship two users ❤️", guild=discord.Object(id=GUILD_ID))
async def ship_slash(interaction: discord.Interaction, user1: discord.Member, user2: discord.Member):
    await do_ship(interaction, user1, user2)

@bot.command(name="ship")
async def ship_prefix(ctx, user1: discord.Member, user2: discord.Member):
    await do_ship(ctx, user1, user2)


TENOR_KEY = os.getenv("TENOR_KEY")  # Add your Tenor API key in Render

# ------------- Tenor GIF Fetcher -------------
async def fetch_tenor_gif(search_term: str):
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://tenor.googleapis.com/v2/search?"
            f"q={search_term}&key={TENOR_KEY}&client_key=discord_bot"
            f"&limit=25&contentfilter=high&media_filter=gif"
        ) as resp:
            data = await resp.json()
            if "results" in data and data["results"]:
                # Try to pick a logical one (prefer anime-related ones)
                for _ in range(5):  # retry a few times
                    choice = random.choice(data["results"])
                    desc = choice.get("content_description", "").lower()
                    if "anime" in desc:
                        return choice["media_formats"]["gif"]["url"]
                # fallback if no anime keyword matched
                return random.choice(data["results"])["media_formats"]["gif"]["url"]
    return None

# ------------- Shared Action Function -------------
async def do_action(ctx_or_inter, action_text: str, search_term: str, member: discord.Member):
    author = ctx_or_inter.user if isinstance(ctx_or_inter, discord.Interaction) else ctx_or_inter.author
    if member is None:
        member = author

    is_self = (member == author)

    self_messages = {
        "hugged 🤗": f"{author.mention} is feeling lonely and hugs themselves 🤗",
        "punched 🥊": f"{author.mention} is practicing punches on the air 🥊",
        "kissed 😘": f"{author.mention} blows a kiss to themselves 😘",
        "slapped 👋": f"{author.mention} accidentally slaps themselves 👋",
        "licked 👅": f"{author.mention} licks themselves 👅 (weird!)",
        "pampered 🌸": f"{author.mention} is pampering themselves 🌸",
        "cuddled 🛏": f"{author.mention} cuddles with their pillow 🛏",
        "loved ❤": f"{author.mention} loves themselves ❤ (self-care!)",
        "killed 💀": f"{author.mention} dramatically pretends to kill themselves 💀"
    }

    msg_text = self_messages.get(action_text) if is_self else f"{author.mention} {action_text} {member.mention}!"

    # Always fetch gifs from Tenor
    gif_url = await fetch_tenor_gif(search_term)

    embed = discord.Embed(description=msg_text, color=discord.Color.random())
    if gif_url:
...
# ------------- Curated Queries for Each Command -------------
SEARCH_TERMS = {
    "hugged 🤗": "anime hug gif",
    "punched 🥊": "anime punch",
    "kissed 😘": "anime kiss with someone scene gif",
    "slapped 👋": "anime slap gif",
    "licked 👅": "anime lick",
    "pampered 🌸": "anime comfort gif",
    "cuddled 🛏": "anime cuddle gif",
    "loved ❤": "anime cuddle gif",
    "killed 💀": "anime fight finish move gif (kill somone)"
}

# ------------- Prefix Commands -------------
@bot.command()
async def hug(ctx, member: discord.Member = None):
    await do_action(ctx, "hugged 🤗", SEARCH_TERMS["hugged 🤗"], member)

@bot.command()
async def punch(ctx, member: discord.Member = None):
    await do_action(ctx, "punched 🥊", SEARCH_TERMS["punched 🥊"], member)

@bot.command()
async def kiss(ctx, member: discord.Member = None):
    await do_action(ctx, "kissed 😘", SEARCH_TERMS["kissed 😘"], member)

@bot.command()
async def slap(ctx, member: discord.Member = None):
    await do_action(ctx, "slapped 👋", SEARCH_TERMS["slapped 👋"], member)

@bot.command()
async def lick(ctx, member: discord.Member = None):
    await do_action(ctx, "licked 👅", SEARCH_TERMS["licked 👅"], member)

@bot.command()
async def pamper(ctx, member: discord.Member = None):
    await do_action(ctx, "pampered 🌸", SEARCH_TERMS["pampered 🌸"], member)

@bot.command()
async def cuddle(ctx, member: discord.Member = None):
    await do_action(ctx, "cuddled 🛏", SEARCH_TERMS["cuddled 🛏"], member)

@bot.command()
async def love(ctx, member: discord.Member = None):
    await do_action(ctx, "loved ❤", SEARCH_TERMS["loved ❤"], member)

@bot.command()
async def kill(ctx, member: discord.Member = None):
    await do_action(ctx, "killed 💀", SEARCH_TERMS["killed 💀"], member)
...
async def hug_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "hugged 🤗", SEARCH_TERMS["hugged 🤗"], member)

@bot.tree.command(name="punch", description="Punch someone 🥊")
async def punch_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "punched 🥊", SEARCH_TERMS["punched 🥊"], member)

@bot.tree.command(name="kiss", description="Kiss someone 😘")
async def kiss_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "kissed 😘", SEARCH_TERMS["kissed 😘"], member)
...
async def slap_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "slapped 👋", SEARCH_TERMS["slapped 👋"], member)

@bot.tree.command(name="lick", description="Lick someone 👅")
async def lick_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "licked 👅", SEARCH_TERMS["licked 👅"], member)

@bot.tree.command(name="pamper", description="Pamper someone 🌸")
async def pamper_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "pampered 🌸", SEARCH_TERMS["pampered 🌸"], member)

@bot.tree.command(name="cuddle", description="Cuddle someone 🛏")
async def cuddle_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "cuddled 🛏", SEARCH_TERMS["cuddled 🛏"], member)

@bot.tree.command(name="love", description="Love someone ❤")
async def love_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "loved ❤", SEARCH_TERMS["loved ❤"], member)

@bot.tree.command(name="kill", description="Kill someone 💀 (playful)")
async def kill_slash(interaction: discord.Interaction, member: discord.Member = None):
    await do_action(interaction, "killed 💀", SEARCH_TERMS["killed 💀"], member)
